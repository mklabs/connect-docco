...
var path = require('path'),
  ...
  docco_template = template(fs.readFileSync(__dirname + '/../support/docco/docco.jst', 'utf8')),

  // Create the template function that we will use to generate container part that is emited on file changes.
  docco_container = template(fs.readFileSync(__dirname + '/../support/docco/container.jst', 'utf8')),

  // The CSS styles we'd like to apply to the documentation.
  docco_styles = fs.readFileSync(__dirname + '/../support/docco/docco.css', 'utf8'),

  // Create the template that we will use to generate landing page.
  index_template = template(fs.readFileSync(__dirname + '/../index.html', 'utf8')),
...
send = exports.send = function send(req, res, next, options) {
  ...
  var parsed = url.parse(options.path, true),
    query = parsed.query,
    uri = decodeURIComponent(parsed.pathname),
    type;
  ...
  if(uri.match(/docco\.css/)) {
    return docco_css(req, res, next);
  }

  if (path.normalize('/') == uri[uri.length - 1]) {
    uri += 'index.html';
    req.url += 'index.html';
  }

  // requested file extension not supported by docco or `?raw` query string, fallback silently.
  if(path.basename(uri) !== 'index.html' && !get_language(uri) || query.raw != null) return next();
  ...
  fs.stat(uri, function(err, stat) {
    ...
    fs.readFile(uri, 'utf8', function(err, content) {
      ...
      var sections = parse(uri, content);
      return highlight(uri, sections, function() {
          var title = path.basename(uri),
          html = (title === 'index.html' ? index_template : docco_template)({
            title: title,
            sections: sections,
            sources: [],
            path: uri,
...
parse = function parse(source, code) {
  ...
  if(!language) {
    console.error("Wasn't able to get language for", source);
    return sections;
  }
